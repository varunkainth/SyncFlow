import jwt, { JwtPayload, Secret, SignOptions, VerifyErrors } from "jsonwebtoken";
import { User, TokenPayload } from "../types/auth.types"; // Ensure correct path
import { ROLES, Role, ALL_PERMISSIONS, Permission } from "../constants/permissions.constants"; // Fixed import path
 ...
 */
const parseExpiry = (value: string | undefined, defaultValue: SignOptions["expiresIn"]): SignOptions["expiresIn"] => {
  if (!value) return defaultValue;
  return !isNaN(Number(value)) ? Number(value) : (value as SignOptions["expiresIn"]);
};

// Token Expiry (Supports both numeric values and time strings like "15m", "7d")
const ACCESS_TOKEN_EXPIRY: SignOptions["expiresIn"] = parseExpiry(process.env.ACCESS_TOKEN_EXPIRY, "15m");
const REFRESH_TOKEN_EXPIRY: SignOptions["expiresIn"] = parseExpiry(process.env.REFRESH_TOKEN_EXPIRY, "7d");

// Generate Access Token (Short-lived, includes role & permissions)
export function generateAccessToken(user: Pick<TokenPayload, "userId" | "email" | "roles" | "permissions">): string {
...
export function verifyAccessToken(token: string): User | null {
  try {
    ...
    // Validate roles
    const validRoles = roles.filter((role): role is Role => ROLES.includes(role as Role));
    
    // Validate permissions
    const validPermissions = (payload.permissions || []).filter((perm): perm is Permission => ALL_PERMISSIONS.includes(perm as Permission));
    ...
    console.error("[JWT] Token verification failed:", (error as VerifyErrors).message);
    return null;
  }
}
...
export function verifyRefreshToken(token: string): { userId: string, jti: string } | null {
  try {
    ...
    console.error("[JWT] Refresh token verification failed:", (error as VerifyErrors).message);
    return null;
  }
}
